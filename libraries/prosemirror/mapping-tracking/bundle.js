(() => {
  ...
  const fo = document.querySelector('#content'),
    uo = document.querySelector('#editor'),
    mo = ge.create({ schema: Mt, doc: rt.fromSchema(Mt).parse(fo) });
  let vo = [];
  const go = new so(uo, {
    state: mo,
    dispatchTransaction(t) {
      const e = go.state.apply(t);
      if (t.docChanged) {
        const e = t.mapping.maps.map(function (t) {
          return t.ranges;
        });
        vo.push(...e), console.log('mappings', vo);
      }
      go.updateState(e);
    },
  });
  document.querySelector('#sync').addEventListener('click', function () {
    ...
    if (!t || t.rangeCount <= 0)
      return void console.error('Please select in the document.');
    const e = t.getRangeAt(0),
      { startContainer: n, startOffset: r, endContainer: o, endOffset: i } = e;
    if (!fo.contains(n) || !fo.contains(o))
      return void console.error('Please select in the document.');
    let s = r + 1,
      a = i + 1;
    const c = new Et(
    ...
    (s = c.map(s)), (a = c.map(a, -1));
    const h = oe.create(go.state.doc, s, a),
      p = go.state.tr;
    p.setSelection(h);
    const l = go.state.apply(p);
    go.updateState(l), go.focus();
  });
})();
