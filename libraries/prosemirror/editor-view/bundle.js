(() => {
  ...
  function Ye(t, e) {
    ...
    if (Oe.safari)
      for (var f = p; o && f; f = Ae(f)) f.draggable && (o = i = null);
    if (
      ((p = (function (t, e) {
        var n = t.parentNode;
        return n &&
  ...
  function vo(t, e) {
    if (t.dragging !== mo) {
      const {
        slice: { openEnd: n, openStart: r },
        move: o,
      } = t.dragging;
      console.log(e.type, { slice: { openEnd: n, openStart: r }, move: o }),
        (mo = t.dragging);
    }
  }
  const go = new so(fo('#editor'), {
    state: uo,
    handleDOMEvents: {
      dragstart: vo,
      drag: vo,
      ...
      dragend: vo,
      keydown: function (t) {
        console.log(t.composing);
      },
    },
  });
  (window.editorView = go),
    fo('#posAtCoords').addEventListener('click', function () {
      const t = Number(fo('#left').value),
        e = Number(fo('#top').value);
      console.log('posAtCoords', go.posAtCoords({ left: t, top: e }));
    }),
    fo('#coordsAtPos').addEventListener('click', function () {
      const t = Number(fo('#pos').value),
        e = Number(fo('#side').value);
      console.log('coordsAtPos', go.coordsAtPos(t, e));
    }),
    fo('#domAtPos').addEventListener('click', function () {
      const t = Number(fo('#pos1').value),
        e = Number(fo('#side1').value);
      console.log('domAtPos', go.domAtPos(t, e));
    }),
    fo('#nodeDOM').addEventListener('click', function () {
      const t = Number(fo('#pos2').value);
      console.log('nodeDOM', go.nodeDOM(t));
    }),
    fo('#posAtDOM').addEventListener('click', function () {
      const t = go.dom.querySelector('p').childNodes[0],
        e = Number(fo('#offset').value),
        n = Number(fo('#bias').value);
      console.log('posAtDOM', go.posAtDOM(t, e, n));
    });
})();
