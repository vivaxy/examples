...
AStar.prototype.findConflicts = function (state, i, dimension) {
  ...
  for (var h = 0; h < state.length - 1 && tilesRelated.indexOf(h) === -1; h++) {
    for (
      var k = h + 1;
      k < state.length && tilesRelated.indexOf(h) === -1;
      k++
    ) {
      var moves =
        dimension == 1
...
var start = function () {
  ...
  panel.innerHTML =
    'Solution: ' + result.path + ' Total steps: ' + result.path.length + '';
...
var showSolution = function () {
  ...
  switch (solution[step]) {
    case 'R':
      move =
        emptytilePosRow.toString() + ',' + (emptytilePosCol + 1).toString();
      break;
    case 'L':
      move =
        emptytilePosRow.toString() + ',' + (emptytilePosCol - 1).toString();
    ...
    case 'U':
      move =
        (emptytilePosRow - 1).toString() + ',' + emptytilePosCol.toString();
      break;
    case 'D':
      move =
        (emptytilePosRow + 1).toString() + ',' + emptytilePosCol.toString();
