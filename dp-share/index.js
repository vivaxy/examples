...
if (location.href.indexOf('product=dpapp') >= 0) {
    ...
    var shareConfig = "dpshare://_?content=" + encodeURIComponent(JSON.stringify(shareData));
    frameContainer.appendChild(iframe);
    document.body.appendChild(frameContainer);
    iframe.setAttribute("src", shareConfig);

    document.querySelector('button').addEventListener('click', function () {
        location.href = 'js://_?method=share&args=' + encodeURIComponent(JSON.stringify(shareData));
    });
} else {
