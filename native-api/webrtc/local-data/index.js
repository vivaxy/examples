...
function createConnection() {
  ...
  localConnection.createOffer().then(gotLocalDescription).catch(onCreateLocalDescriptionError);
}
...
function trace(message) {
  console.log((Math.floor(performance.now()) / 1000).toFixed(3) + ': ' + message);
}
...
function gotLocalDescription(desc) {
  ...
  localConnection.setLocalDescription(desc).then(onSetLocalDescriptionSuccess).catch(onSetLocalDescriptionError);
  remoteConnection.setRemoteDescription(desc).then(onSetRemoteDescriptionSuccess).catch(onSetRemoteDescriptionError);

  trace('Sending remote answer');
  remoteConnection.createAnswer().then(onCreateAnswerSuccess, onCreateAnswerError);
}
...
function onCreateAnswerSuccess(desc) {
  ...
  remoteConnection.setLocalDescription(desc).then(onSetLocalDescriptionSuccess).catch(onSetLocalDescriptionError);
  localConnection.setRemoteDescription(desc).then(onSetRemoteDescriptionSuccess).catch(onSetRemoteDescriptionError);
}
